...
class res_partner(models.Model):
	...
	@api.multi
	def cleanup(self):
		if self.supplier_history > 0:
			records =self.env['lubon_suppliers.import_stats'].search([('supplier_id','=',self.id)], order="start desc", offset=self.supplier_history)
			for s in records:
				s.unlink()
		...
		with open (stats.supplier_id.supplier_file, 'rb') as cleanfile:
			...
			for row in reader:
				...
				newline= {'stats_id':stats.id, 
					...
					'purchase_price':row[fieldlist['price_purchase']].replace(" ","").replace(".","").replace(",","."),
					}
